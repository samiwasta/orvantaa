...
export function SidebarInsetHeader({
  ...
  return (
    <header className="flex min-h-14 shrink-0 items-center gap-2 px-3 py-3 pt-4 sm:gap-3 sm:px-4 sm:pt-5 md:px-6 md:pt-6">
      {!isMobile ? <SidebarTrigger className="-ml-1 shrink-0" /> : null}
      <div className="flex min-w-0 flex-1 items-center justify-between gap-2">
        <h1
          className={cn(
            "m-0 min-w-0 truncate pb-0.5 font-heading text-base leading-snug font-semibold tracking-tight text-foreground sm:text-lg"
          )}
        >
          {pageTitle}
        </h1>
